/******
 *
 * 'NP hw3 rwg server (modify the hw2 rwg connection-oriented server)'
 * by hEaDhSu
 *
 ******/
...
struct ExecCmd {
    ...
    /*** 0:none(default), 1:|, 2:!, 3:>, 4:>|, 5:>!, 6:<<EOF ***/
    int pipeType;
};
struct ClientInfo {
...
void SigChild(int signo) {
    if(childPid > 0) {
        int i = 0;
        for(; i <= *maxi; ++i) {
            //printf("user:%d, fd: %d, isConnect:%d\n", i+1, client[i].fd, client[i].isConnect);fflush(stdout);
...
void OpenShell(char* buf, int n) {
    ...
    strtok(buf2, "\r");
    strtok(buf2, "\n");
    /*write(stdoutfd, "parse cmd line: ", 16);
    write(stdoutfd, buf2, strlen(buf2));
    write(stdoutfd, "\n", 1);*/
    ...
    for(; j < cmdNum && client[curClientNo].isConnect; ++j) {
        ...
        if(ParseCmdAndArgv(execCmdQueue[curCmdLineNo].tmpExecCmdLine, execCmd, execArgv, &cmdLen, &argvNum, &execCmdQueue[curCmdLineNo].pipeType) == -1) break;
...
void ParsePipe(char* buf, struct ExecCmd* execCmdQueue, int* cmdNum) {
    ...
    char* ptrPipe = buf;
    int cmdLineLen = 0;
    for(; (ptrTmp = strpbrk(ptrTmp, "|!>")) && *cmdNum < CMD_MAX_NUM; *cmdNum = *cmdNum + 1) {
        if( *ptrTmp == '|') {
        ...
        ++ptrTmp;
        ptrPipe = ptrTmp;
        /*write(stdoutfd, "add '", 5);
        write(stdoutfd, execCmdQueue[*cmdNum].execCmdLine, cmdLineLen);
        write(stdoutfd, "' to execCmdQueue\n", 18);*/
    }
...
int ParsePipeCheck(char* ptrPipe) {
    char tmpBuf[BUF_MAX_SIZE];
    char* ptrBuf;
    strcpy(tmpBuf, ptrPipe);
    if( !(ptrBuf = strtok(tmpBuf, " "))) {
        write(STDERR_FILENO, "ParsePipeCheck error!\n", 22);
        return 0;
    }
    if(!strcmp(ptrBuf, "exit")) return 1;
    if(!strcmp(ptrBuf, "printenv")) return 1;
    if(!strcmp(ptrBuf, "setenv")) return 1;
    if(!strcmp(ptrBuf, "who")) return 1;
    if(!strcmp(ptrBuf, "name")) return 1;
    if(!strcmp(ptrBuf, "yell")) return 1;
    if(!strcmp(ptrBuf, "tell")) return 1;
    return 0;
}
...
int ReadLine(char** buf, int* n) {
    ...
    while( (result = read(client[curClientNo].fd, &c, 1)) == 1 ) {
        //fprintf(stderr, "%c (%d), \n", c, c);fflush(stdout);
        if(c == '\r') continue;
        if(i == BUF_MAX_SIZE - 1) {
            write(stdoutfd, "\nread overflow!\n", 16);
