...
void hvm_hlt(unsigned long rflags)
{
	...
	/** itri cpu performance code */
    if (curr->domain->cpu_monitor_flag)
    {
        curr->domain->last_halt_time = NOW();
    }
                                 ...
                                 bool_t access_x)
{
    ...
    if(p2mt == p2m_ram_monitor){
        p2m->set_entry(p2m, gfn, mfn, 0, p2m_ram_rw, p2m->default_access);
        v->domain->used_counter++;
        return 1;
    }
