...
int start_sample(int domID){
    ...
    page_list_for_each(pg, &d->page_list){
        ...
        if(mfn_valid(mfn)){
            ...
            p2m_change_type(p2m, gfn, p2mt, p2m_ram_monitor);
            (d->total_pages)++;
        }
    }

    printk("<VT> start smaple lock pages : %lu\n", total_pages); 
    domain_unpause(d);

    return 1;
}
int end_sample(int domID){
    struct domain *d = get_domain_by_id(domID);
    struct page_info *pg;
    struct p2m_domain *p2m;
    unsigned long mfn, gfn;
    ...
    page_list_for_each(pg, &d->page_list){
        ...
        if(mfn_valid(mfn)){
            gfn = mfn_to_gfn(d, mfn);
            gfn_to_mfn_query(p2m, gfn, &p2mt);
            if(p2mt == p2m_ram_monitor){ 
                p2m_change_type(p2m, gfn, p2mt, p2m_ram_rw);
            }
        }
    }
...
unsigned long do_change_ept_content(int domID, unsigned long gfn, unsigned long mfn, int flag, void *buff)
{
    ...
    switch(flag){
        ...
        case 2:   
            ...
            break;
        case 3:
            /*Copy analyze_data to Dom0*/
            longBuff[0] = d->used_counter;
            longBuff[1] = d->total_pages;
            break;       
        case 5:
            d->used_counter = 0;
            d->total_pages = 0;
        ...
        case 6:
            for(i=0; i<RECENT_CR3_SIZE; i++){
                printk("<VT>recent_cr3[%lu]:%lx\n", i, d->recent_cr3[i].cr3);
            }
            break;
        case 7:
            /*walk cr3 in array*/
        ...
        case 8:
            ...
            for(i=0; i<RECENT_CR3_SIZE; i++){
                if(d->test_cr3[i].cr3!=0){
                    printk("<VT> cr3:%lx swap_num:%lu swap_new_num:%lu\n", 
                            d->test_cr3[i].cr3,
                            d->test_cr3[i].swap_num,
