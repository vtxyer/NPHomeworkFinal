...
*   @desc used to save sampling data
*/
...
pRecord DataTable[1024]; //< default # of domain is 1024
pRecord TailEntry[1024]; //< point to tail entry

/**
*   Active this flag to sample CPU data
*       0 is off, otherwise is on
*/
int cpu_monitor_flag[1024] = {0};


/** 
*   Update the sampling data
*/
void updateDataTable(domid_t domid, u64 cr3, u64 rip,
                        u64 rsp, u64 rbp)
{
    pRecord newRecord;
    struct domain *dom = get_domain_by_id(domid);
    
    if (dom == NULL) return;

    spin_lock(&dom->cpu_monitor_lock);
    
    /// construct new record
    newRecord = xmalloc( record_t );
    /// no enough memory space, replace the oldest data with the new one
    if (newRecord == NULL)
    {
        printk("[updateTable] Error while monitoring domain %d...\n", domid);

        if (DataTable[domid] != NULL)
        {
            /// only one entry
            if (DataTable[domid] == TailEntry[domid])
            {
                DataTable[domid]->cr3 = cr3;
                DataTable[domid]->rsp = rsp;
                DataTable[domid]->timestamp = NOW();
            }
            else
            {
                pRecord temp = DataTable[domid];
                
                temp->cr3 = cr3;
                temp->rsp = rsp;
                temp->timestamp = NOW();

                DataTable[domid] = temp->next;

                /// Attach to tail
                temp->next = NULL;
                TailEntry[domid]->next = temp;
                TailEntry[domid] = temp;
            }
        }

        spin_unlock(&dom->cpu_monitor_lock);
        return;
    }
    newRecord->cr3 = cr3;
    newRecord->rsp = rsp;
    ...
    /// no data in the table, newRecord is the head of the table
    if (DataTable[domid] == NULL) 
    {
        DataTable[domid] = newRecord;
        TailEntry[domid] = newRecord;
    }
    /// already exist some data
    else 
    {
        TailEntry[domid]->next = newRecord;
        TailEntry[domid] = newRecord;
    }
...
/**
*   Hypercall - do_cpu_monitor
*   @param op
*       0 turn on cpu_monitor_flag
*       1 turn off cpu_monitor_flag
*       2 dequeue a sampling data
*   @param domid
*       domain id
*   @param data
*       the sampling data will be copied out into the mem space
*/
int do_cpu_monitor(int op, domid_t domid, void *data)
{
    if (op == 0)
    {
        struct domain *dom = get_domain_by_id(domid);
        if (dom == NULL) return -1;
        dom->cpu_monitor_flag = 1;
        spin_lock_init(&dom->cpu_monitor_lock);

        printk("Domain %d start monitoring...\n", domid);
        DataTable[domid] = NULL;
        TailEntry[domid] = NULL;
    }
    else if (op == 1)
    { 
        struct domain *dom= get_domain_by_id(domid);
        ...
        DataTable[domid] = TailEntry[domid] = NULL;
        
        printk("Domain %d stop monitoring!\n", domid);
    }
    else if (op == 2)
    {
        pRecord ptr, copy_out = (pRecord) data;
        struct domain *dom= get_domain_by_id(domid);
        if (dom == NULL) return -1;
        ...
        if (ptr != NULL)
        {
            copy_out->cr3 = ptr->cr3;
            copy_out->rsp = ptr->rsp;
            copy_out->timestamp = ptr->timestamp;

            // delete this data which has been copied out
            DataTable[domid] = ptr->next;
            if (DataTable[domid] == NULL) TailEntry[domid] = NULL;
            xfree(ptr);
        }
        /// set the cr3 to 0, it represents no data
        else copy_out->cr3 = 0;
    ...
    {
        s_time_t *copy_out = (s_time_t*) data;

        struct domain *dom= get_domain_by_id(domid);
        
        if (dom == NULL) return -1;
        
        *copy_out = NOW() - dom->last_halt_time;
    }
...
static void schedule(void)
{
    ...
    if( next->domain->swap_sample_start >= 0 ){
        spin_lock(&(next->domain->recent_cr3_lock));
        for(i=0; i<RECENT_CR3_SIZE; i++){
            if(tmp_cr3 == next->domain->recent_cr3[i].cr3){  //cr3 already in list                
                break; 
            }
        }   
        if(i==RECENT_CR3_SIZE){ //add new cr3 into list
            for(i=RECENT_CR3_SIZE-1; i>0; i--){
                next->domain->recent_cr3[i].cr3 = next->domain->recent_cr3[i-1].cr3;
            }
            next->domain->recent_cr3[0].cr3 = tmp_cr3;
            next->domain->recent_cr3[0].swap_num = 0;
        }
        spin_unlock(&(next->domain->recent_cr3_lock));
    }
    if( next->domain->swap_sample_start == 1 ){
        spin_lock(&(next->domain->test_cr3_lock));
        for(i=0; i<RECENT_CR3_SIZE; i++){
            if( tmp_cr3 == next->domain->test_cr3[i].cr3 
	...
	dump_regs = next->arch.guest_context.user_regs;
	/// if the domain's cpu_monitor_flag is not 0, add new data
	if (next->domain->cpu_monitor_flag != 0)
	{
		updateDataTable(next->domain->domain_id,
						next->arch.hvm_vcpu.guest_cr[3],
						dump_regs.rip,
						dump_regs.rsp,
						dump_regs.rbp);
	}
