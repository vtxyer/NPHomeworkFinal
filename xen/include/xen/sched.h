...
#define NR_DOMAIN_WATCHDOG_TIMERS 2
    ...
    /** CPU performance monitor flag */
    bool_t cpu_monitor_flag;

    /** last time of halt inst invoked */
    s_time_t last_halt_time;

    /** update data spin lock */
    spinlock_t cpu_monitor_lock;
     ...
     * bits
     * 0: hit
     * 1: swap_bit  0->in non-swap, 1->in swap
     * 2:
     * 3:
     * 4:
     * 5:
     ...
     * s*/
    char *hit_gfn_ary;  
    unsigned long used_counter;
    unsigned long total_pages;

#define RECENT_CR3_SIZE 25
    struct recent_cr3 recent_cr3[RECENT_CR3_SIZE]; //size related to schedule.c
    struct recent_cr3 test_cr3[RECENT_CR3_SIZE]; //size related to schedule.c
    int swap_sample_start;
    int os_type; // 0->linux  1->windows
    unsigned long total_swap_num;

    unsigned long non2a;
    unsigned long a2non;
    spinlock_t set_mem_counter_lock;
    spinlock_t recent_cr3_lock;
    spinlock_t test_cr3_lock;
    int testFlag;
